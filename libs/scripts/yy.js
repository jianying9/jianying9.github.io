...
//                data = data.replace(that._pattern, '');
    ...
    var response = {
        ...
        readLocal: function(msg) {
            ...
            var info = this._services.message[msg.flag];
            if (!info) {
                var service = this._services.get(msg.act);
                if (service) {
                    info = service.message[msg.flag];
                } else {
                    info = msg.flag;
                }
            }
            msg.info = info;
            this._notify(this._root, msg);
        }
    };

    YY.response = response;
